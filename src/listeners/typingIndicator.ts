...
const typingIndicators: Collection<string, {message: Message, timer: NodeJS.Timeout}> = new Collection();
...
setInterval(async () => {
  ...
  const message = await typingIndicators.at(currentlyAnimating)?.message.fetch().catch(() => {return; });
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    if (message.content.slice(2, frame.length + 3) == frame + " ") {
      ...
      message.edit("**" + frames[nextFrame] + message.content.slice(frame.length + 2)).catch(() => {return; })
      break;
    }
  }
}, 1500)
  ...
  events: [
    {
      ...
      execute(message: Message) {
        ...
        if (typingIndicators.has(key)) {
          typingIndicators.get(key)?.message.delete().catch(() => {return;});
      ...
      async execute(typing: Typing) {
        ...
        if (currentTypingIndicator != null) {
          ...
          message = await typing.channel.send({
            content: "**" + frames[0] + " " + typing.member?.displayName + "** is typing...",
            allowedMentions: {
              parse: []
            },
